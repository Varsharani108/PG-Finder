...
import { getOwnerDashboard, updateInquiryStatus } from "../../api/ownerApi.js";
...
export default function OwnerDashboard() {
  ...
  const content = () => {
    ...
    if (tab === "bookings") return <section className="rounded-xl border border-primary/10 bg-white p-5"><h2 className="text-xl font-semibold text-primary">Bookings & Tenants</h2><p className="mt-1 text-sm text-primary/60">Live bookings belonging to your properties.</p>{bookings.length ? <div className="mt-5 space-y-3">{bookings.map((booking) => <div key={booking._id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-primary/10 p-4"><div><p className="font-medium text-primary">{booking.tenant?.name || "Unknown tenant"}</p><p className="text-sm text-primary/60">{booking.property?.name || "Property removed"} · {booking.tenant?.phone || ""}</p></div><span className="rounded-full bg-primary/5 px-3 py-1 text-xs font-semibold text-primary">{booking.status}</span><strong className="text-primary">₹{(booking.rent || 0).toLocaleString("en-IN")}</strong></div>)}</div> : <p className="py-8 text-center text-sm text-primary/60">No bookings or tenants found.</p>}</section>;
